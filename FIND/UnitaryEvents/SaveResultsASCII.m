...
SaveNameAscRates = [d.OutPath d.OutFileName 'Rates.asc'];
...
disp(['... save rates to ' SaveNameAscRates]);
save(SaveNameAscRates,'ratemat','-ascii');
 ...
 if isempty(r.Results.ExistingPatterns)
  ...
  for i=1:length(pat_idx)
   % string generation of pattern
     ...
     patString
     SaveNameAscPat = [d.OutPath d.OutFileName patString '.asc'];
   ...
   disp(['... save results of ' patString ' to ' SaveNameAscPat]);
   save(SaveNameAscPat,'resultmatrix','-ascii');
