...
% Last Modified by GUIDE v2.5 18-Nov-2005 15:24:46
...
set(fig,'Color',get(0,'DefaultUicontrolBackgroundColor'));
    ...
    information(['The "Unitary Events Batch Mode" allows UE Analysis of several ' ...
                 ...
                 'list can be saved or loaded in the File menu.'],'b');
    drawnow;
...
% filelist{i} replaced by filelist(i)


 % get filenames
 filelist = cellstr(get(handles.listbox,'string'));
 if isequal(filelist,{''})
     information('No Parameter Files selected.','r');
     drawnow;
 else
     for i=1:length(filelist)
         ...
         if ~isempty(DataFile);
            ...
            information(['Could not load valid DataFile structure from ' filelist(i)],'r');
            drawnow;
...
function varargout = remove_pushbutton_Callback(h, eventdata, handles, varargin)
    ...
    information(['Removed Parameter File: ' oldlist{position}],'b');
    drawnow;
  ...
  if ~(filename == 0)
    ...
    if ~isempty(DataFile)
       ...
       if ~isequal(filelist,{''})
          ...
          set(handles.remove_pushbutton,'enable','on');
          information(['Loaded new Batch Script: ' loadfilename],'b');
          drawnow;
       else
          information(['Could not load valid file list from' loadfilename],'r');
          drawnow;  
       end
    else
       information(['Could not load valid DataFile structure from' loadfilename],'r');
       drawnow;
    end
...
function varargout = batchsave_submenu_Callback(h, eventdata, handles, varargin)
 ...
 if ~(filename == 0)
  ...
  filelist = cellstr(get(handles.listbox,'string'));
  if isequal(filelist,{''})
      information('No Parameter Files in List. Cannot save empty Batch Script.','r');
      drawnow;
  else
      for i=1:length(filelist)
          % load DataFile structure
      ...
      information(['Saved Batch Script to File: ' savefilename],'b');
      drawnow;
  end
 end
