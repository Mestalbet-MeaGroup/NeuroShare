function ReadMcdAndPrepDataNoah3(file,Params)
...
segmententityIDs = segmententityIDs([EntityInfo(segmententityIDs).ItemCount] > 1);
...
%      waitbar(c/length(channels),h,sprintf('%12.1f',percent));
     ...
     segments=(1:10:EntityInfo(i).ItemCount);
     segments(end)=EntityInfo(i).ItemCount;
     
    for window=1:length(segments)-1
        
        [~, TimeData, VoltageData ,~, ~] = ns_GetSegmentData(hfile,i, segments(window):segments(window+1));
