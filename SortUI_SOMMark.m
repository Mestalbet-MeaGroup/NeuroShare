...
if sort==1,
    ...
    [pathstr,name,ext] = fileparts(DataStr.folders{1});
    St2=[pathstr '\'];
...
if N>0,
    ...
    N=size(Coefs,2);
    fid=fopen([fname,dext],'r'); D=fscanf(fid,'%d',240*di); fclose(fid);
    if 240*di == size(D,1)
        display(fname);
...
if channel_fid ~=-1
    ...
    while ~isempty(channel) & channel ~=-1
        ...
        eval (['load channel',num2str(channel),';']);
        %eval (['load5 channel',num2str(channel),';']);

        eval(['classes=Classes',num2str(channel),';']);

        if classes~=0,
            eval(['NT=T',num2str(channel),'(1:9,:);']);
            eval(['NTG=TG',num2str(channel),';']);
            eval(['sMap=sMap',num2str(channel),';']);
            eval(['best_k=best_k',num2str(channel),';']);
            eval(['clusters=clusters',num2str(channel),';']);
            ...
            if classes(1)~=0,
                ...
                if ~isempty(tt)
                    for i=1:classes
                        ...
                        tindexchannel(1,i)=str2num(channel);
                        tindexchannel(2,i)=i;
...
fclose(fid);
fclose(ifid);
tt1=sort(tt1(tt1>237));
tt2=sort(tt2(tt2>237));
tt3=sort(tt3(tt3>237));
tt4=sort(tt4(tt4>237));
tt5=sort(tt5(tt5>237));
tt6=sort(tt6(tt6>237));
tt7=sort(tt7(tt7>237));
tt8=sort(tt8(tt8>237));
tt9=sort(tt9(tt9>237));
tt10=sort(tt10(tt10>237));
...
for i=1:BinNum,
    HistData(i,:)=hist(data1(data2>Ybins(i)-dBinY & data2<Ybins(i)+dBinY),Xbins);
end
HistData=HistData;
figure('name','Density');
%figure;
...
if ~isempty(NeuronIndex)
   ...
   t=t.*10000/(25000*2.68).*12000; %strange unit conversion. Not sure what the original units were...
end