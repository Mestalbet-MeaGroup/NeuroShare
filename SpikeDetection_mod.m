...
function SpikeDetection_mod() %,SpikeDetectionThresh,RemoveSubThreshSpikes,AddWaveletCoeff)

[fileToRead1,filepath] = uigetfile('*.mcd');
McdFile=[filepath fileToRead1];
MinSpikesInIndexChannel=5;
Params.SpikeDetectionThresh=50e-7;
...
co=0;
for x=1:length(SelectedChannels)
    i=SelectedChannels(x);
    co=co+1;
    [sss, TimeData, VoltageData , ssss, sssss] = ns_GetSegmentData(hfile,i, 1:EntityInfo(i).ItemCount);
    Params.SpikeDetectionThresh=GetChannelThreshold(hfile,i);
    clear sss;
    clear ssss;
    clear sssss;
